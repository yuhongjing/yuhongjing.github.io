...
```
    function parseUrl(url){
        ...
        let dataArr = url.substr(pos+1).split('&');
        let map = {};
        for(let i=0; i<dataArr.length; i++){
            let pos1 = dataArr[i].indexOf('=');
            if(pos1 !== -1){
                let key = dataArr[i].substr(0, pos1);
                let data = dataArr[i].substr(pos1+1);
                map[key] = data;
            }
        }
        return map;
...
### promise封装ajax
```
    let getPromise = function (url, options){
        let promise = new Promise(function (resolve, reject){
            let client = new XMLHttpRquest();
            ...
            if(options.methods == "GET"){
                ...
                for(let k in options.header){
                    client.setRequestHeader(k, options.header[k]);
                }
